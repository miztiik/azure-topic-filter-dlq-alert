...
def _get_n_set_app_config(credential):
    try:
        ...
        client = AzureAppConfigurationClient(
            GlobalArgs.APP_CONFIG_URL, credential=credential)

        GlobalArgs.SA_NAME = client.get_configuration_setting(
            key="saName").value
        GlobalArgs.BLOB_NAME = client.get_configuration_setting(
            key="blobName").value
        GlobalArgs.Q_NAME = client.get_configuration_setting(
            key="queueName").value
...
def _read_n_del_from_q(q_name, q_svc_client, _evnt_type, blob_svc_client):
    try:
        ...
        for msg in messages:
            ...
            # Slow down the consumer to build up the queue
...
def lambda_handler(event, context):
    ...
    blob_svc_client = BlobServiceClient(
        GlobalArgs.BLOB_SVC_ACCOUNT_URL, credential=credential, logging=azure_log_level)
    q_svc_client = QueueServiceClient(
        GlobalArgs.Q_SVC_ACCOUNT_URL, credential=credential, logging=azure_log_level)
